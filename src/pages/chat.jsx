...
import { Button, Form, Input, Popover, Skeleton, Tooltip, message } from 'antd'
...
const Chat = ({chatId, user}) => { 
  ...
  const {data:oldMessge ,setData : setOldMessage} = useInfiniteScrollTop(containerRef ,
  ...
  const allMessages = [...oldMessge , ...messages]
...
if(bottomRef.current)
  ...
  const fileChangeHandler = async (e,key)=>{
    ...
    if(files.length > 5) return message.error(`connot send more than 5 ${key}`)
     ...
     dispatch(setIsUploadingLoader(true))
      ...
      try{
        ...
        if(res.data){ message.success(`${key} succesfully send`, {id:messageID})}

        else { message.error(`${key} not send error` ,{id : messageID})}
        console.log(res.error)

      }catch(err){
        message.error(err ,{id:messageID})
      }finally{
